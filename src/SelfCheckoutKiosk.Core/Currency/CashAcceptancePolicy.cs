...
public sealed class CashAcceptancePolicy
{
    ...
    public CashAcceptanceResult Evaluate(
        ...
        decimal maxAllowedOverpaymentKhr = 100m)
    {
        ValidateKhrAmount(
            targetKhr,
            nameof(targetKhr)
        ...
        if (
            proposedPaidKhr.Amount >
            targetKhr.Amount
        )
        {
            var overpaymentKhr = proposedPaidKhr.Amount - targetKhr.Amount;
            if (overpaymentKhr <= maxAllowedOverpaymentKhr)
            {
                /*
                 * The note is accepted because the overpayment is within
                 * the allowed tolerance (e.g. 100 KHR / small change rounding).
