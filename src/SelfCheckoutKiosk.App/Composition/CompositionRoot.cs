...
public sealed class KioskServices
{
    ...
    public required KioskDatabaseInitializer DatabaseInitializer { get; init; }
    public required string DatabasePath { get; init; }
    public required string DatabaseEncryptionKey { get; init; }
}
...
public static class CompositionRoot
{
    ...
    public static KioskServices Build()
    {
        ...
        IOfflineLicenseManager licenseManager = new FallbackDevelopmentLicenseManager(
        ...
        var databaseInitializer = new KioskDatabaseInitializer(dbContextFactory);
        ...
        return new KioskServices
        {
            ...
            DatabaseEncryptionKey = PlaceholderDatabaseEncryptionKey
        };
    }

    private sealed class FallbackDevelopmentLicenseManager : IOfflineLicenseManager
    {
        private readonly IOfflineLicenseManager _inner;

        public FallbackDevelopmentLicenseManager(IOfflineLicenseManager inner)
        {
            _inner = inner;
        }
        ...
        public async Task LoadAndValidateAsync(CancellationToken cancellationToken = default)
        {
            try
            {
                await _inner.LoadAndValidateAsync(cancellationToken);
            }
            catch (LicenseValidationException)
            {
                System.Diagnostics.Debug.WriteLine("[DEV-WARNING] No valid offline license found. Falling back to development license mode.");
            }
        }
        ...
        public void EnforceFeatureAccess(LicensedFeature feature)
        {
            try
            {
                _inner.EnforceFeatureAccess(feature);
            }
            catch (LicenseValidationException)
            {
                // Permit all features in development/testing mode
            }
        }
    }
}
