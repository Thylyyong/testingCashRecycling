...
| 🟢 | `HIGH` | **Vault Breakdown Cross-Session Persistence** | Automatic JSON persistence for physical KHR/USD banknote counts in `VaultInventoryService`; retained across app restarts unless manually reset by store attendants. | [VaultInventoryService.cs](file:///c:/Users/viti/source/repos/SelfCheckoutKiosk-V2%20-%20Merge/src/SelfCheckoutKiosk.App/Services/VaultInventoryService.cs) |
| 🟢 | `HIGH` | **Cart Dialog Physical Keyboard & Enter Support** | Direct hardware keyboard, numpad, and barcode typing into Add Item Manually, Check Price, and Recall Cart modals with Enter key execution. | [CartView.xaml.cs](file:///c:/Users/viti/source/repos/SelfCheckoutKiosk-V2%20-%20Merge/src/SelfCheckoutKiosk.App/Views/Customer/CartView.xaml.cs) |
...
| 🟢 | `HIGH` | **Dynamic Media & Branding Sync & Persistence** | Live store logo, company name, tagline, and store hours integration between `MediaBrandingView` and `HomeView`. Playlist custom sort order and Active/Inactive toggle states persist across app restarts. Per-video sound toggle button (`&#xE767;` volume on / `&#xE74F;` mute) with dynamic icon feedback and persistent audio state for kiosk video playback. Runtime deleted default items restored on startup. JSON persistence in `branding_config.json`. | [MediaBrandingService.cs](file:///c:/Users/viti/source/repos/SelfCheckoutKiosk-V2%20-%20Merge/src/SelfCheckoutKiosk.App/Services/MediaBrandingService.cs) · [MediaBrandingView.xaml](file:///c:/Users/viti/source/repos/SelfCheckoutKiosk-V2%20-%20Merge/src/SelfCheckoutKiosk.App/Views/Admin/MediaBrandingView.xaml) |
| 🟢 | `HIGH` | **Peripheral & Workflow Deduplication** | Scan deduplication (400ms debounce guard + single-pipeline dispatch), elimination of redundant CartView wedge listener to prevent duplicate +2 item addition, Khmer store hours localization, and SuccessView single-print enforcement. | [App.xaml.cs](file:///c:/Users/viti/source/repos/SelfCheckoutKiosk-V2%20-%20Merge/src/SelfCheckoutKiosk.App/App.xaml.cs) · [CartView.xaml.cs](file:///c:/Users/viti/source/repos/SelfCheckoutKiosk-V2%20-%20Merge/src/SelfCheckoutKiosk.App/Views/Customer/CartView.xaml.cs) · [SuccessView.xaml.cs](file:///c:/Users/viti/source/repos/SelfCheckoutKiosk-V2%20-%20Merge/src/SelfCheckoutKiosk.App/Views/Customer/SuccessView.xaml.cs) |
| 🟢 | `HIGH` | **Global Sound System & Audio Feedback Engine** | Centralized dual-channel audio engine with strongly-typed `KioskSound` enum, pre-cached `MediaPlayer` channels for SFX and voice prompts, and 1-line `AppSound` facade across all screens. | [SoundService.cs](file:///c:/Users/viti/source/repos/SelfCheckoutKiosk-V2%20-%20Merge/src/SelfCheckoutKiosk.App/Services/Audio/SoundService.cs) · [AppSound.cs](file:///c:/Users/viti/source/repos/SelfCheckoutKiosk-V2%20-%20Merge/src/SelfCheckoutKiosk.App/Services/Audio/AppSound.cs) |
| 🟢 | `HIGH` | **Admin Login ContentDialog & Key Isolation** | Replaced full-page `AdminLoginView` navigation with a floating `ContentDialog` launched from every Admin button and `Ctrl+Shift+A`. Integrated 3x4 keypad with pixel-perfect width alignment across input, touch pad, and action buttons. `AdminLoginViewModel` strictly enforces 8-digit PIN validation (`DefaultAdminPin` / `88888888`), preventing premature Enter execution. Fixed event listener leaks by properly scoping and removing key handlers on dialog dismiss. Added `IsTabStop="False"` to customer `AdminButton` elements. | [AdminLoginDialog.cs](file:///c:/Users/viti/source/repos/SelfCheckoutKiosk-V2%20-%20Merge/src/SelfCheckoutKiosk.App/Services/AdminLoginDialog.cs) · [AdminLoginViewModel.cs](file:///c:/Users/viti/source/repos/SelfCheckoutKiosk-V2%20-%20Merge/src/SelfCheckoutKiosk.App/ViewModels/Admin/AdminLoginViewModel.cs) · all `*View.xaml.cs` |
| 🟢 | `HIGH` | **Distribution Packaging Script & Launcher Resilience** | Standalone packaging in `Package-Distribution.ps1` made resilient to directory locks by cleaning contents without deleting directory nodes, killing lingering kiosk processes, wrapping in `try / catch / finally` with interactive pause prompt, and providing 1-click `Package-Distribution.bat` launcher. | [Package-Distribution.ps1](file:///c:/Users/viti/source/repos/SelfCheckoutKiosk-V2%20-%20Merge/Package-Distribution.ps1) · [Package-Distribution.bat](file:///c:/Users/viti/source/repos/SelfCheckoutKiosk-V2%20-%20Merge/Package-Distribution.bat) |
| 🟢 | `MEDIUM` | **CartView Toolbar Button Sounds** | `AppSound.ButtonClick()` wired to all 7 CartView chip buttons (Add Item, Check Price, Recall, Save Cart, USD/KHR, Help, Admin) and all dialog action buttons. | [CartView.xaml.cs](file:///c:/Users/viti/source/repos/SelfCheckoutKiosk-V2%20-%20Merge/src/SelfCheckoutKiosk.App/Views/Customer/CartView.xaml.cs) |
| 🟢 | `MEDIUM` | **F11 Fullscreen Toggle** | `F11` key in `Global_PreviewKeyDown` toggles `AppWindowPresenterKind.FullScreen` ↔ `Default`. Works from any screen. | [MainWindow.xaml.cs](file:///c:/Users/viti/source/repos/SelfCheckoutKiosk-V2%20-%20Merge/src/SelfCheckoutKiosk.App/MainWindow.xaml.cs) |
| 🟢 | `LOW` | **Payment Card Layout & Viewport Size-Based Responsive Redesign** | Replaced fragile screen-wide `AdaptiveTrigger` with dynamic viewport size listening (`SizeChanged` / `ActualWidth` vs `ActualHeight`). Strictly locks to **2 cards per row (2×3)** for all vertical/portrait kiosk resolutions (540×960, 1080×1920) and only expands to **3 cards per row (3×2)** when the app window itself is wide horizontal (`Width > Height && Width >= 1200`). Eliminated dummy spacer rows/columns, added low-priority layout re-measurement, and fixed WinUI 3 Release-mode transition clipping. | [PaymentOptionView.xaml.cs](file:///c:/Users/viti/source/repos/SelfCheckoutKiosk-V2%20-%20Merge/src/SelfCheckoutKiosk.App/Views/Customer/PaymentOptionView.xaml.cs) · [PaymentSelectionView.xaml.cs](file:///c:/Users/viti/source/repos/SelfCheckoutKiosk-V2%20-%20Merge/src/SelfCheckoutKiosk.App/Views/Customer/PaymentSelectionView.xaml.cs) |
| 🟢 | `LOW` | **SuccessView Done Button Redesign** | Added leading ✓ icon, transparent footer background, and refined spacing for a more polished action button. | [SuccessView.xaml](file:///c:/Users/viti/source/repos/SelfCheckoutKiosk-V2%20-%20Merge/src/SelfCheckoutKiosk.App/Views/Customer/SuccessView.xaml) |
