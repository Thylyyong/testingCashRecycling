...
| 🟢 | `HIGH` | **Admin Login ContentDialog** | Replaced full-page `AdminLoginView` navigation with a floating `ContentDialog` launched from every Admin button and `Ctrl+Shift+A`. Integrated 3x4 keypad with pixel-perfect width alignment across input, touch pad, and action buttons. `AdminLoginViewModel` reused with Khmer font support. | [AdminLoginDialog.cs](file:///c:/Users/viti/source/repos/SelfCheckoutKiosk-V2%20-%20Merge/src/SelfCheckoutKiosk.App/Services/AdminLoginDialog.cs) · all `*View.xaml.cs` |
